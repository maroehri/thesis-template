\chapter{Tipps zur Nutzung von LaTeX}
...
Es ist wichtig, auf die manuelle Verwendung von \mintinline{text}{\\} zu
...
\mintinline{text}{\\} zur Strukturierung des Fließtextes kann zu
unvorhersehbarem Layout führen und widerspricht der semantischen Textgliederung,
die \LaTeX\ von Haus aus unterstützt.

Stattdessen sollten Absätze durch den logischen Einsatz von Leerzeilen im
...
\textit{geschützter dünner Leerraum} (\mintinline{text}{\,}) verwendet wird.
Beispielsweise wird "\mintinline{text}{z.\,B.}" anstelle von
"\mintinline{text}{z. B.}" geschrieben. Dies sorgt dafür, dass der Abstand
optisch angenehmer wirkt und den typografischen Standards entspricht.

In \LaTeX\ wird das Tilde-Symbol (\mintinline{text}{~}) verwendet, um zwischen
...
"\mintinline{text}{S.~42}" für Seitenangaben oder
"\mintinline{text}{Prof.~Dr.}". Der geschützte Leerraum hat die gleiche Breite
...
\begin{itemize}
  \item Der einfache Bindestrich \mintinline{text}{-} wird für die Trennung von
  Wörtern verwendet, wie in "Auto-Industrie".
  \item Der Geviertstrich \mintinline{text}{--} dient als Gedankenstrich oder
  zur Trennung von Zahlenbereichen, z.\,B. "Seiten 10--20" oder "Heute -- nicht
  morgen."
  \item Der lange Strich \mintinline{text}{---} wird für längere Gedankenstriche
...
Sie können nebem dem Titel auch die Schrift über einen optionale Option
\texttt{font} beeinflussen, z.\,B. wie im folgenden:

\mintinline{text}{\begin{mybox}[title=Bildschirmausgabe, font=\small]{console}}

\begin{mybox}[title=Bildschirmausgabe, font=\small]{console}
Hier dann der Text in kleinerer Festbreitenschrift
\end{mybox}
...
Es gibt außerdem die Möglichkeit, einzelne Abschnitte rot oder grün zu
hinterlegen mit den Kommandos \mintinline{text}{\highlightred{}} bzw.
\mintinline{text}{\highlightgreen{}} wie in folgendem Beispiel dargestellt:

\begin{mybox}[title=semester.md, font=\small]{file}
[...]

### Software-Technik

- \highlightgreen{Allgemeine }Einführung
- \highlightgreen{Übersicht }Vorgehensmodelle
- Agile Softwareentwicklung
\highlightred{- Requirements Engineering} 
\highlightgreen{- Business Analysis} 
\end{mybox}



\section{Die awesomebox-Umgebung}

Demonstration der Nutzung einer \texttt{awesomebox}-Umgebung:

\begin{awesomeblock}[hse-hellblau]{2pt}{\faInfo}{hse-hellblau}
\small\textit{Wichtiger Hinweis}: beachten Sie die Option "Initialize repository with
a README". Diese Option sollte nur angewählt werden, wenn Sie ein neues
Repository durch GitLab/GitHub anlegen lassen wollen. Sofern Sie bereits ein
lokales Repository haben, dass Sie nach GitLab/GitHub pushen wollen, sollten Sie
die Option \textit{nicht} wählen (sonst müssten Sie später mit 

\texttt{git merge origin/main --allow-unrelated-histories}

die beiden Historien vereinen)
\end{awesomeblock}




\section{Ein macOS-Terminal}
